...
13. [Editing DNA e Style Extraction Engine](#13-editing-dna-e-style-extraction-engine)
...
### K Clipper
**Papel:** decidir o destino de cada momento (Short / vídeo longo / descarte) e montar os recortes preliminares.
...
**Ferramentas:** WhisperX (faster-whisper + wav2vec2, BSD-2) com timestamps por palavra, permitindo legendas animadas palavra-a-palavra. Estilo (fonte, cor, animação, posição) definido pelo Editing DNA do canal.
...
**Ferramentas:** FFmpeg com `-filter_complex` para composição de camadas, overlays animados, shake, zoom cinético e transições; MoviePy como abstração. Todos os assets vêm da K Library — o agente não gera efeitos do zero, ele seleciona, parametriza e aplica recursos existentes conforme o Editing DNA.
...
A camada de thumbnail é 100% programática, sem nenhum modelo de geração de imagem.
...
O Kreator foi projetado desde a concepção como um sistema distribuído de agentes especializados. Dezenas de K Agents podem participar de um mesmo projeto, mas nenhum deles tem autoridade para modificar livremente o fluxo de trabalho. Toda execução segue uma arquitetura coordenada, com responsabilidades, permissões e dependências rigidamente definidas — de modo que a colaboração entre agentes seja determinística, previsível e reproduzível.

### Uma responsabilidade por agente

Cada K Agent tem uma única responsabilidade principal, que nunca se sobrepõe à de outro agente. O K Analyst analisa, mas não edita nem publica. O K Editor organiza a timeline, mas não cria estratégia nem altera SEO. O K Subtitle controla apenas legendas. O K Motion controla apenas movimento, zoom e transições. O K SEO produz apenas metadados e nunca toca no vídeo. Essa separação reduz conflitos e torna o comportamento do sistema previsível.

### O K Orchestrator

Toda comunicação é coordenada por um agente central, o **K Orchestrator** — o equivalente ao kernel do sistema. Ele não executa tarefas criativas; sua função é exclusivamente coordenar os demais agentes: iniciar processos, controlar permissões, distribuir tarefas, resolver dependências, validar entradas e saídas, sincronizar execução, impedir conflitos e monitorar falhas.

Nenhum agente se comunica diretamente com outro. Toda troca passa pelo K Orchestrator, o que elimina decisões concorrentes e mantém o sistema consistente.

### O Workflow como contrato imutável

Antes de qualquer processamento, o Kreator constrói um Workflow completo, que funciona como um contrato entre todos os agentes. Ele define a ordem das etapas, os responsáveis, as dependências, as permissões, os ativos utilizados e as regras específicas do projeto. Durante a execução, todos os agentes seguem esse Workflow, e nenhum pode alterar unilateralmente sua estrutura.

Vale distinguir dois sentidos do termo. Este Workflow é o **plano de execução** daquele projeto, montado pelo K Orchestrator. Ele é gerado a partir da configuração do canal e, quando o criador licencia um **K Workflow** do marketplace (a metodologia de edição descrita na [Seção 17](#17-k-store--marketplace)), os parâmetros dessa metodologia alimentam o plano de execução. Um é a receita reutilizável; o outro é a execução concreta daquele vídeo.

### The K Protocol

A coordenação obedece a um princípio único, o **K Protocol**:

> Nenhum agente altera diretamente o trabalho de outro. Cada agente produz um artefato estruturado — análise, timeline, metadados, recomendação ou validação — que é verificado pelo K Orchestrator antes de ser consumido pelo próximo agente.

Esse protocolo aproxima o Kreator de uma arquitetura de microsserviços: em vez de um conjunto de prompts encadeados, o sistema é uma rede de serviços que trocam artefatos versionados e validados. É esse fundamento que permite à automação escalar sem se tornar caótica.

### Sistema de permissões

Cada agente possui permissões explícitas, concedidas pelo princípio do menor privilégio. O K Analyst pode ler vídeo e áudio e escrever análise, mas não pode editar timeline, exportar ou publicar. O K Editor pode ler a análise e editar a timeline, mas não pode alterar SEO nem publicar. Permissões nunca são compartilhadas entre agentes sem necessidade.

### Personalização sem conflito

Workflows personalizados de criadores não substituem os agentes internos do Kreator — apenas fornecem parâmetros adicionais. Um Workflow que define "cortes rápidos, zoom intenso, legenda central e meme a cada 45 segundos" não cria um novo editor: ele modifica o comportamento do K Editor, que continua sendo o responsável pela edição. Isso impede que configurações de terceiros alterem componentes críticos da plataforma.

### Sandbox de execução

Cada projeto roda em um ambiente isolado. Os agentes trabalham apenas sobre os arquivos daquele projeto, e nenhuma alteração afeta outros usuários ou outros Workflows. O isolamento garante reprodutibilidade, segurança e estabilidade.

### Determinismo e reprodutibilidade

O Kreator busca resultados consistentes: dois projetos executados com o mesmo vídeo, o mesmo Workflow, as mesmas bibliotecas, as mesmas configurações e a mesma versão dos agentes devem produzir resultados equivalentes.

A precisão do termo importa. Com modelos de linguagem e visão rodando em GPU, a identidade bit-a-bit é inviável — ponto flutuante e amostragem introduzem variação. A promessa do Kreator é de **reprodutibilidade auditável**, obtida com seeds fixos, temperatura zero onde aplicável e registro completo de versões. O objetivo não é gerar exatamente os mesmos pixels, e sim permitir auditoria, depuração e evolução controlada.

### Versionamento

Todos os componentes são versionados. Cada projeto registra a versão do Kreator, dos K Agents e dos Workflows, além das bibliotecas, modelos e parâmetros utilizados. Qualquer projeto pode ser reproduzido no futuro com exatamente a mesma configuração — base para o modelo de imutabilidade descrito na seção seguinte.

---

## 9. Trust Layers e Immutable Core

A integridade do Kreator se apoia em uma separação rígida de camadas de confiança. Componentes descartáveis e componentes permanentes nunca se misturam.
...
```
                ...
                K Orchestrator
                       │
──────────────────────────────────────────────
   Project Sandboxes (isolados, destruíveis)
──────────────────────────────────────────────
   Shared Services Layer
   K Memory · K Library · K Models · K Workflows · K Registry
──────────────────────────────────────────────
   Immutable Core Layer
   Model Registry · Workflow Registry · Asset Registry
   Security Keys · Audit Logs
──────────────────────────────────────────────
              Storage Layer
```

Apenas os Sandboxes podem ser destruídos. O Core, nunca.

### Project Sandbox

Cada vídeo enviado gera um ambiente isolado que contém os agentes daquele projeto, arquivos temporários, timeline, caches, estados e logs. Quando o projeto termina, o Sandbox pode ser destruído, e nenhum outro projeto consegue acessá-lo.

### Shared Services

Aqui ficam os serviços compartilhados — K Memory, K Library, K Models, K Workflows, K Registry. Todos os agentes podem ler; pouquíssimos podem escrever.

### Immutable Core

O núcleo da plataforma não pode ser modificado por nenhum agente, plugin, Workflow ou usuário. Modelos, Workflows, bibliotecas e ativos não são alterados em produção: são **substituídos por novas versões**.

O princípio que governa toda a arquitetura é:

> **Everything is Replaceable. Nothing is Mutable.**
> *Tudo pode ser substituído. Nada deve ser alterado diretamente.*

Na prática, uma biblioteca nunca recebe arquivos novos por cima. Em vez disso, gera-se uma nova versão fechada, e o sistema apenas troca a referência ativa:

```
Current Library → v4      (rollback → v3, sem reconstruir nada)
```

Se algo der errado, o rollback é a simples troca da referência de volta para a versão anterior. É o mesmo modelo de imutabilidade que sustenta Git, NixOS, Docker e Kubernetes: em vez de alterar o estado existente, cria-se um novo estado e redireciona-se o sistema para ele.

### Armazenamento endereçado por conteúdo

O identificador de cada Workflow, biblioteca ou modelo é o **hash do seu próprio conteúdo**. Esse único mecanismo resolve, de uma vez, versionamento, deduplicação, verificação de integridade e rollback: se o conteúdo muda, o identificador muda; se dois ativos têm o mesmo hash, são o mesmo ativo. Tudo é referenciado por UUID, checksum e assinatura — nunca por nome.

### K Registry

Os agentes nunca procuram arquivos diretamente. Eles consultam o **K Registry** — Workflow Registry, Agent Registry, Plugin Registry, Model Registry, Asset Registry, Dataset Registry — e recebem o componente correto pelo seu ID. Isso elimina o acesso direto ao armazenamento e torna cada dependência rastreável.

### Banco de dados versionado

Registros nunca são sobrescritos. Uma atualização de um Workflow não altera a versão existente: cria a versão seguinte, e a anterior continua existindo. Isso permite auditoria completa do histórico de qualquer componente.

### K Security — Verificação de Integridade

A verificação de integridade é responsabilidade de um agente dedicado, o **K Security**. Ele não edita vídeos: verifica assinaturas digitais, hashes, permissões, isolamento e acesso. Quando um editor publica um Workflow no marketplace, o componente recebe SHA-256, assinatura digital, versão e autor; no momento do uso, o K Security confirma que é exatamente o Workflow original, íntegro e não modificado.

A distinção de papéis é clara: o K Validator cuida da autenticidade do **conteúdo** (Reality First, zero imagens geradas, apenas assets registrados), enquanto o K Security cuida da integridade do **sistema** (hashes, assinaturas, permissões, isolamento). São duas preocupações distintas, atribuídas a dois agentes distintos.

Essa arquitetura tem uma consequência direta de segurança: se um atacante comprometer um agente, o máximo que alcança é um Sandbox. Modelos, Workflows, memória central, bibliotecas e ativos permanecem intactos, porque são somente leitura.
...
## 13. Editing DNA e Style Extraction Engine
...
Não treina IA. **Observa** o trabalho real de um editor e extrai parâmetros objetivos a partir de vídeos editados, projetos originais e arquivos de timeline (Premiere, DaVinci, Final Cut):
...
Esse conjunto de parâmetros vira o **Editing DNA** — a identidade criativa digital do editor. Se um criador quer vídeos "no estilo do Editor João", o sistema carrega o DNA daquele editor e executa a edição seguindo exatamente aquela identidade.
...
**2. Editing DNA do marketplace.** Um criador novo pode licenciar o **K Workflow** de um editor experiente e obter "bons defaults" instantâneos, sem nenhum histórico próprio. Isso entrega valor imediato e desacopla o produto do aprendizado lento.
...
- **Fase 2 — Editores parceiros:** convite a 10–15 editores conhecidos para criar os primeiros Editing DNAs.
...
Todo o valor do MVP está na **orquestração** — não nos modelos individualmente.
...
3. **K Store — marketplace de Editing DNA.** Um modelo de negócio que transforma o conhecimento de editores em ativo licenciável. Nenhum dos concorrentes tem isso — e é o que cria um ecossistema de dois lados (criadores + editores).
...
**Stack:** LangGraph (checkpointing via PostgreSQL); Qwen2.5-VL-7B via vLLM; PySceneDetect; auto-editor + FFmpeg; WhisperX; Pillow + OpenCV + rembg + MediaPipe (thumbnails 100% programáticas); Qwen3 via Ollama; pgvector; Docker Compose; Redis Streams.
...
**Stack:** Mem0 + Letta; migração pgvector → Qdrant; Neo4j + GraphRAG; Arize Phoenix + statsmodels/scipy; Prophet + AutoGluon; Fish Speech / XTTS v2; multi-node com workers GPU dedicados; Grafana + Prometheus.
...
**Stack:** Kubernetes; Kafka; pipelines de anotação; MLflow; CUDA para fine-tuning; dados anotados com consentimento explícito.
...
A especialização acontece por configuração e memória — não por treinamento. O `channel_profile.json` codifica preferências de centenas de vídeos; o feedback ajusta parâmetros; o K Scientist identifica combinações vencedoras; o Style Extraction Engine transforma histórico de edições em parâmetros. Nenhuma GPU de treinamento. O sistema aprende pelo uso.
...
- Estilo extraído de centenas de projetos reais
...
**Critério de sucesso:** criadores reconhecem e valorizam a camada de sistema operacional como motivo de escolha.
